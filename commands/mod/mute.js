...
module.exports = {
   ...
   run: async (bot, message, args) => {
       try {
           if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("**You Dont Have Permmissions To Mute Someone! - [KICK_MEMBERS]**");
         ...
         try {
           mutee.roles.set([muterole.id]).then(() => {
               mutee.send(`**Hello, You Have Been Muted In ${message.guild.name} for - ${reason || "No Reason"} Proof: ${proof}`).catch(() => null)
           })
           } catch {
               ...
               message.channel.send(sembed);
                    ...
                    if (!proof)
            sembed3 = new MessageEmbed()
                   .setColor("GREEN")
                   .setDescription(`You must have proof.`)
